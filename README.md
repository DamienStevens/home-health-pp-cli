...
Because AirThings/IQAir are latest-only, run `collect` on a schedule. On macOS, a launchd agent firing every ~10 minutes keeps a dense history without manual runs. Example `~/Library/LaunchAgents/com.example.home-health-collect.plist`:
...
<dict>
  <key>Label</key>
  <string>com.example.home-health-collect</string>
...
Load it with `launchctl load ~/Library/LaunchAgents/com.example.home-health-collect.plist`. (Adjust the binary path to wherever `home-health-pp-cli` is installed.)
